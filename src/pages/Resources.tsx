    ...
    <PageLayout>
      ...
      <Section className="pt-24 md:pt-32">
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-[220px_1fr] items-start">
          <Reveal delay={0.1}>
            <div className="flex flex-col items-center md:items-start">
              ...
              <h3 className="text-lg font-semibold mt-3">{about.founder.name}</h3>
        ...
        <div className="mt-8 border-t border-border/60 pt-6">
          <Reveal delay={0.1}>
            ...
            <p className="text-base font-medium text-foreground/90">{about.company.description}</p>
          ...
          <Reveal delay={0.15}>
            <div className="mt-8">
              <p className="text-xs font-semibold text-foreground uppercase tracking-wider mb-4">What We Do</p>
                    ...
                    >
                      <div className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-3">
                          <span className="text-lg">{pillarIcons[i]}</span>
          ...
          <Reveal delay={0.2}>
            <div className="mt-10">
              <p className="text-xs font-semibold text-foreground uppercase tracking-wider mb-5">How We Work</p>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {about.company.process.map((s, i) => (
                  <HoverCard key={s.step} className="p-4 text-center">
                    <span className="text-2xl font-bold text-accent block">{s.step}</span>
                    <p className="text-sm font-semibold text-foreground mt-1">{s.title}</p>
          ...
          <Reveal delay={0.25}>
            <div className="mt-8 grid gap-2 sm:grid-cols-2">
              {about.company.outcomes.map((o) => (
                <div key={o.title} className="flex items-start gap-2 text-sm py-1.5">
                  <CheckCircle className="h-4 w-4 text-accent shrink-0 mt-0.5" />
                  <span><span className="font-medium text-foreground">{o.title}</span> <span className="text-muted-foreground">— {o.desc}</span></span>
                </div>
              ))}
            </div>
          </Reveal>
      ...
      <Section className="border-t border-border/60">
        <Reveal>
          ...
          <p className="text-muted-foreground mb-6 text-sm">From trades to tech — structured systems for every stage.</p>
        </Reveal>
            ...
            >
              <div className="grid gap-6 md:grid-cols-3">
                {(["problem", "solution", "result"] as const).map((key) => (
                  <div key={key}>
                    <p className="text-xs font-semibold text-accent uppercase tracking-wider mb-2">{key}</p>
                    <p className="text-sm text-muted-foreground leading-relaxed">{caseStudies[activeCase][key]}</p>
                  </div>
                ))}
              </div>
            </motion.div>
